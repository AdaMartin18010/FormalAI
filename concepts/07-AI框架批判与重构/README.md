...
- **SKNO（Schrödinger化的核神经算子）**（2025年5月）：Haoze Song等人在论文《Redefining Neural Operators in $d+1$ Dimensions》中，提出了在$d+1$维空间中重新定义神经算子的方法。他们通过引入新的演化机制，设计了Schrödinger化的核神经算子（SKNO），在各种基准测试和零样本超分辨率任务中表现出色。该研究为神经算子理论提供了新的数学框架，展示了在更高维度空间中定义神经算子的可能性（arXiv:2505.11766）

- **共享神经空间（Shared Neural Space）**（2025年9月）：Jing Li等人在论文《Shared Neural Space: Unified Precomputed Feature Encoding for Multi-Task and Cross Domain Vision》中，提出了共享神经空间的概念。该方法通过统一的预计算特征编码，实现了多任务和跨领域视觉任务的高效处理，减少了冗余，提高了模型的泛化能力。该研究展示了神经算子理论在多任务学习中的应用潜力（arXiv:2509.20481）

- **Aurora模型**（2025年）：微软的Aurora模型利用3D Swin Transformer，在海量异构数据上进行预训练，不仅能够进行天气预报，还能高精度预测空气污染等多物理场耦合问题，展示了神经算子在多领域应用中的巨大潜力。该模型展示了神经算子理论在实际应用中的成功案例
