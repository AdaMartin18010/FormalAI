...
- [01.2.2-Prompt 工程与 ReAct 循环](#0122-prompt-工程与-react-循环)
  ...
  - [七、2025 年 Prompt 工程与 ReAct 技术趋势](#七2025-年-prompt-工程与-react-技术趋势)
    - [7.1 2025年最新技术突破](#71-2025年最新技术突破)
  - [七、2025 年 Prompt 工程与 ReAct 技术趋势](#七2025-年-prompt-工程与-react-技术趋势-1)
  - [八、核心结论](#八核心结论)
  - [九、相关主题](#九相关主题)
...
1. **GPT-5.2思考模式**（2025年12月）
   - **核心突破**：OpenAI GPT-5.2提供即时和思考两种模式
   - **技术特点**：
     - 即时模式：快速响应，适合简单任务
     - 思考模式：专注于推理能力提升，适合复杂任务
     - 双模式设计：根据任务复杂度自动选择模式
   - **理论意义**：展示了推理能力提升的新路径
   - **工程意义**：为用户提供不同场景下的最优模式选择

2. **Gemini 2.5 Pro超长上下文**（2025）
   - **核心突破**：100万token上下文窗口
   - **技术特点**：
     - 超长上下文：支持100万token上下文
     - 推理能力提升：在数学、科学等推理任务上表现出色
     - 超长记忆：展示超长记忆对推理能力的提升
   - **理论意义**：探索超长上下文对模型能力的影响
   - **工程意义**：实现更复杂的推理任务，提升模型实用性

3. **SimpleVLA-RL框架**（2025）
...
4. **记忆摊销推理（MAI）框架**（2025年8月）
   - **核心突破**：将认知建模为对记忆中潜在循环的推理（arXiv:2508.14143）
   - **技术特点**：
     - 结构化重用：最小化熵，实现上下文感知和结构保留的推理
     - 记忆循环：对记忆中潜在循环的推理，而非通过梯度下降重新计算
     - 生物学合理性：为实现人工通用智能（AGI）提供了统一且生物学上合理的理论基础
   - **理论意义**：为推理增强提供新的理论框架
   - **工程意义**：提升推理效率，降低计算成本

5. **计算理性（CR）用户模型**（2025年11月）
   - **核心突破**：针对有偏信念下进行最优决策的认知受限代理（arXiv:2511.12359）
   - **技术特点**：
     - 有界记忆：显式建模有界记忆过程如何导致动态不一致和有偏信念状态
     - 序列决策：影响序列决策的质量
     - 用户适配：为开发适应用户记忆限制的AI助手提供了理论支持
   - **理论意义**：为AI助手设计提供新的理论框架
   - **工程意义**：提升AI助手的用户适配能力
...
- [01.4.1-三层协同机制](01.4.1-三层协同机制.md) - Prompt工程与三层协同
...
5. **2025年最新研究**：
   ...
   - **计算理性（CR）用户模型** (2025年11月): 引入一类计算理性用户模型，专门针对在有偏信念下进行最优决策的认知受限代理（arXiv:2511.12359）
   - **STP（自玩定理证明器）** (2025): 突破大型语言模型（LLM）训练数据瓶颈。通过让LLM同时扮演猜想者和证明者，动态生成适应性课程，克服了高质量定理证明数据稀缺的问题
